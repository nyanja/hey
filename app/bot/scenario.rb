  ...
  class Scenario
    ...
    def handle_results
      ...
      results.each_with_index do |result, i|
        break if i > Config.results_count.to_i && pseudo.empty? && target_presence
        if result.text.match?(Config.ignore)
      ...
      if !target_presence && Config.query_skip_on_presence?
        ...
        Logger.info "Запрос отложен на #{Config.query_skip_interval} мин."
        Storage.set query, Time.now.to_i
      elsif target_presence && target_presence <= (Config.query_skip_on_position || 0)
    ...
    def delayed_query?
      ...
      drv.close
      Logger.skip! "Запрос отложен. Осталось #{Config.query_skip_interval - time} мин."
      w = Config.query_delay
      Logger.wait w
      sleep w
      true
    ...
    def visit_some_link
      nav = drv.find_element(class: Config.nav_classes.sample)
    ...
    def scroll
      ...
      drv.scroll_by Config.scroll_amount
    end
